...
| `app/src/composables/` | `useMetronome.js`（Web Audio 前瞻调度）、`useTuner.js`（麦克风 + ACF 基频检测）、`usePracticeTimer.js`（计时器） |
| `app/src/components/Icon.vue` | 线性 SVG 图标组件（瑞士风图标集，别再用 emoji） |
| `歌曲文件/`、`视频教程/` | 用户的歌曲与成田电吉他课程视频，**已 git 忽略**，勿提交 |

Git 历史（3 个提交）：`dbd6c39` 初始（文档+spike）→ `c862756` M1 实现 → `07c9e76` 瑞士军刀风改版。
...
**没有技术阻塞，在等用户验收 M1。** 用户刚看完改版外观，原话「剩下的改完再看」——他正在检查：配色比例（红色会不会太重）、字号留白、返回键位置，以及剩余页面（打卡流程、歌曲库、设备档案）。**不要在他反馈前自作主张改设计**，等他给意见再动手。

开发服务器在 `http://localhost:4174` 后台跑着（vite dev）。本会话结束后该进程可能被系统回收——如果用户说打不开，先跑 `cd F:/电吉他学习/app && npm run dev -- --port 4174` 再排查。

## 5. 下一步计划（按路线图）

1. **等用户反馈** → 按意见微调 CSS/交互（都是小改动）。
