...
25. **录音功能（v0.4.0 新增）**：①录音存 IndexedDB（`recordingsDb.js`），meta 与 blob 分两个 object store，列表页只读 meta——不要把音频全量塞 localStorage（5MB 上限）也不要让 getAll 拖回全部 blob；②MediaRecorder 的 mimeType 要先 `isTypeSupported` 探测（webm/opus → webm → mp4 依次降级），iPhone Safari 只支持 mp4/aac；③分析录音用 `decodeAudioData` 后混单声道再喂 `analyzeAudio`（引擎内部会重采样，不用自己 resample）；只分析前 3 分钟保证秒出；④**songs store 的 getter 叫 `allSongs`、歌名字段叫 `title`**（不是 songs/name），本会话在这里踩过真 bug（关联歌曲下拉空）；⑤麦克风需要安全上下文（HTTPS），与调音器同坑；⑥IAB 浏览器验收时「录音中」每秒 UI 更新会让 Playwright 点击永远等不到元素稳定（超时），改用新 tab + dom_cua/cua 坐标点击；IAB 里 getUserMedia 可能直接放行（能真录环境噪音，测出低置信度 BPM 是正常的，不是 bug）。⑦**webm 录音没有时长字段**（用户反馈的进度条 bug）：`audio.duration` 为 Infinity、进度条拖不动——保存前用 `fix-webm-duration` 补写 Duration（毫秒单位）；**该包是纯 CJS，动态 import 不会自动进 Vite 预构建，dev 下会静默失败**——`vite.config.js` 的 `optimizeDeps.include` 已列入，别再删；验收读 duration 用 `String(el.duration)`（IAB 安全层把非有限值序列化成 undefined）。⑧**v-for 里的模板 ref 会被 Vue 收集成数组**（不是元素）——回听页 audio 的 `ref="audioEl"` 导致 `audioEl.value.pause()` 抛 TypeError、播放中删除/停止全部失效（用户反馈「确认删除没反应」）。**v-for 内要拿元素必须用函数式 ref**（`:ref="setEl"` + `let el = null; function setEl(e){ el = e }`）。连带教训：`audioEl.value?.play()` 这类可选链调用若对象是数组会静默抛错被 catch 吞掉，自动播放从未生效都没人发现——**两段式按钮的确认态变化会重渲染列表，放大这类竞态**；回听页删除已改为独立弹层确认。
