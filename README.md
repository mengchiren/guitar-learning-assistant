...
- ⏰ **练琴提醒**：应用内定时提醒。
- 🎨 **外观主题**（v0.7.x）：主题切换系统——「经典瑞士军刀」默认款 + **「呆唯 · 轻音海洋」**（奶油暖粉 + 珊瑚粉 + Q 版平泽唯看板娘）+ **「鲸鱼女仆 · 深海茶会」**（柔雾蓝 + 水彩鲸鱼云团 + 女仆看板娘，DSH 社区设计风格）；可选的**透明毛玻璃模式**（卡片半透明 + 背景模糊，背景插画更透）；「我的」页一键切换，全站 CSS 变量换肤，新增主题只加数据。
