...
- 🎸 **设备设置建议**：把套路模板（**6 套**：清音伴奏/清音+合唱氛围/轻过载节奏/失真节奏 Riff/**金属 Riff**/失真主音 Solo）映射到具体设备参数；两种显示模式——「新手模式」是大白话分步操作流程（**实物写实面板图**（v0.17.2 按用户实物照片绘制：音箱银灰拉丝面板+黑身琥珀顶旋钮+蜂窝网罩+LCD+双踏板+通道指示灯，吉他白珠光琴身+白护板+HSS 拾音器；**要动的旋钮主题色高亮圈 + 数值指示刻线** + 参数速览 + 照做步骤 + 可先不动的旋钮），「完整模式」是全部参数表。
- 📖 **音箱入门**（v0.6.2，v0.17.0 面板写实化）：工具页入口，JAM BUDDY 2 面板全景点按图解——每个旋钮/拨杆/踏板/指示灯/功能区/接口点一下看说明（含 Bass/Save 这类多功能按压提示）+ 6 套音色套路对照表。
...
- 🎙 **录音回听**：练习页/工具页录音（存本机 IndexedDB，不出设备），录完自动测 BPM 对拍（置信度 + 手动改）并给出**节奏稳定度评分**（拍格偏差 ms 与贴合率，陪练反馈），回听播放/删除，分类与打卡一致。
...
│       │                 # AmpPanel（音箱写实面板 SVG，实物照片对照绘制）、GuitarPanel（吉他写实示意 SVG）、
...
│       └── views/        # 页面
├── .github/workflows/    # CI：合成音频引擎 + 规则 25 项 + 数据 836 项 + 冒烟 25 项 + 设备建议 53 项 + 面板 17 项 + 谱面渲染 19 项 + 壁纸 26 项 + 构建
